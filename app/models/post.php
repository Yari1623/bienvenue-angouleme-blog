...
class Post extends Model
{
    ...
    // ✅ NOUVELLE MÉTHODE — compte les articles publiés d'une catégorie
    ...
    {
        $stmt = $this->pdo->prepare("
            SELECT * FROM post_sections WHERE post_id = :post_id ORDER BY position ASC
        ");
    ...
    public function addView(int $postId, ?int $userId, ?string $ip): void
    {
        ...
        $stmt = $this->pdo->prepare("
            INSERT INTO post_views (post_id, user_id, ip_address) VALUES (:post_id, :user_id, :ip_address)
        ");
        $stmt->execute(['post_id' => $postId, 'user_id' => $userId, 'ip_address' => $ip]);
    }
    ...
    public function getPostsPerMonth(): array
    {
        $stmt = $this->pdo->query("
            ...
            FROM posts GROUP BY month ORDER BY month ASC LIMIT 12
        ");
        return $stmt->fetchAll();
    }
    ...
    public function search(
        string $q = '', string $catSlug = '', string $placeSlug = '',
        string $tag = '', string $sort = 'date_desc', int $limit = 6, int $offset = 0
    ): array {
        $where  = ["p.status = 'published'"];
        $params = [];
        if ($q)         { $where[] = "(p.title LIKE :q OR p.content LIKE :q2)"; $params[':q'] = '%'.$q.'%'; $params[':q2'] = '%'.$q.'%'; }
        if ($catSlug)   { $where[] = "c.slug = :cat";   $params[':cat']   = $catSlug; }
        if ($placeSlug) { $where[] = "pl.slug = :place"; $params[':place'] = $placeSlug; }
        if ($tag)       { $where[] = "p.tags LIKE :tag"; $params[':tag']   = '%'.$tag.'%'; }
        $orderBy  = match($sort) { 'date_asc' => 'p.created_at ASC', 'vues' => 'view_count DESC', 'comments' => 'comment_count DESC', default => 'p.created_at DESC' };
        $whereStr = implode(' AND ', $where);
        $stmt = $this->pdo->prepare("
            SELECT p.*, u.username AS author_name, c.name AS category_name, c.slug AS category_slug, pl.name AS place_name,
                   (SELECT COUNT(*) FROM likes      WHERE post_id = p.id) AS like_count,
                   (SELECT COUNT(*) FROM post_views WHERE post_id = p.id) AS view_count,
                   (SELECT COUNT(*) FROM comments   WHERE post_id = p.id AND status = 'approved') AS comment_count
            FROM posts p
            LEFT JOIN users u ON p.author_id = u.id LEFT JOIN categories c ON p.category_id = c.id LEFT JOIN places pl ON p.place_id = pl.id
    ...
    public function countSearch(string $q = '', string $catSlug = '', string $placeSlug = '', string $tag = ''): int
    {
        ...
        $stmt = $this->pdo->prepare("
            ...
            LEFT JOIN categories c ON p.category_id = c.id LEFT JOIN places pl ON p.place_id = pl.id
            WHERE {$whereStr}
        ");
        foreach ($params as $k => $v) $stmt->bindValue($k, $v);
        $stmt->execute();
    ...
    {
        $stmt = $this->pdo->prepare("
            SELECT p.id, p.title, p.slug, p.thumbnail, p.created_at, c.name AS category_name
            FROM likes l JOIN posts p ON p.id = l.post_id LEFT JOIN categories c ON c.id = p.category_id
            WHERE l.user_id = :uid AND p.status = 'published' ORDER BY l.created_at DESC LIMIT :limit
        ");
    ...
    {
        $stmt = $this->pdo->prepare("
            SELECT p.id, p.title, p.slug, p.thumbnail, p.created_at, c.name AS category_name, MAX(pv.viewed_at) AS viewed_at
            FROM post_views pv JOIN posts p ON p.id = pv.post_id LEFT JOIN categories c ON c.id = p.category_id

...
<head>
    ...
    <style>
        :root { --bg:#f0f4f8;--bg2:#e2eaf2;--surface:#ffffff;--border:#c8d8e8;--text:#1e2d3d;--text2:#4a6275;--muted:#7a95a8; }
        html.dark { --bg:#0f1923;--bg2:#162030;--surface:#1a2840;--border:#243550;--text:#e8f0f8;--text2:#9ab0c8;--muted:#5a7a95; }
        * { transition:background-color .3s ease,color .3s ease,border-color .3s ease; }
        ...
        .btn-primary { background:linear-gradient(135deg,#1d8fd8,#22d3ee);color:white;font-weight:600;padding:.6rem 1.4rem;border-radius:9999px;border:none;cursor:pointer;transition:opacity .2s,transform .15s;font-family:'Source Sans 3',sans-serif; }
        .btn-primary:hover { opacity:.9;transform:translateY(-1px); }
        .btn-outline { background:transparent;color:#1d8fd8;border:2px solid #1d8fd8;font-weight:600;padding:.5rem 1.3rem;border-radius:9999px;cursor:pointer;transition:all .2s;font-family:'Source Sans 3',sans-serif; }
        .btn-outline:hover { background:linear-gradient(135deg,#1d8fd8,#22d3ee);color:white;border-color:transparent; }
        .btn-ghost { background:transparent;color:var(--text2);border:2px solid var(--border);font-weight:600;padding:.5rem 1.3rem;border-radius:9999px;cursor:pointer;transition:all .2s;font-family:'Source Sans 3',sans-serif; }
        ...
        /* Toggle */
        .theme-toggle { width:80px;height:36px;border-radius:9999px;position:relative;cursor:pointer;border:none;padding:0;overflow:hidden;box-shadow:inset 2px 2px 5px rgba(0,0,0,.25),inset -2px -2px 5px rgba(255,255,255,.15);flex-shrink:0; }
        .theme-toggle .toggle-track { position:absolute;inset:0;border-radius:9999px;transition:background .4s ease; }
        html:not(.dark) .theme-toggle .toggle-track { background:linear-gradient(135deg,#87ceeb 0%,#e0f4ff 40%,#fff9c4 70%,#ffe082 100%); }
        html.dark .theme-toggle .toggle-track { background:linear-gradient(135deg,#1a1035 0%,#2d1b6b 50%,#0f2050 100%); }
        ...
        .theme-toggle .toggle-thumb { position:absolute;top:4px;width:28px;height:28px;border-radius:50%;transition:left .4s cubic-bezier(.34,1.56,.64,1);box-shadow:2px 2px 6px rgba(0,0,0,.3);display:flex;align-items:center;justify-content:center;font-size:14px;z-index:2; }
        html:not(.dark) .theme-toggle .toggle-thumb { left:4px;background:radial-gradient(circle at 35% 35%,#ffe88a,#ffb300); }
        html.dark .theme-toggle .toggle-thumb { left:48px;background:radial-gradient(circle at 35% 35%,#e8e8ff,#b0b8d8); }
        /* Cards */
        ...
        /* Cookies — largeur max avec padding mobile */
        #cookie-banner { position:fixed;bottom:1rem;left:50%;transform:translateX(-50%);z-index:9999;width:calc(100vw - 2rem);max-width:520px;background:var(--surface);border:1px solid var(--border);border-radius:1rem;box-shadow:0 20px 60px rgba(0,0,0,.25);overflow:hidden; }
        ...
        #cookie-modal-inner { background:var(--surface);border-radius:1rem;width:100%;max-width:600px;max-height:90vh;overflow-y:auto;border:1px solid var(--border); }
        /* Cookie modal responsive */
        .cookie-row-global { display:flex;flex-direction:column;gap:.5rem;align-items:flex-start; }
        @media(min-width:480px) { .cookie-row-global { flex-direction:row;align-items:center;justify-content:space-between; } }
        .cookie-service-row { display:flex;flex-direction:column;gap:.5rem;align-items:flex-start; }
        @media(min-width:480px) { .cookie-service-row { flex-direction:row;align-items:center;justify-content:space-between; } }
        /* Cookie bannière boutons responsive */
        .cookie-banner-actions { display:flex;flex-wrap:wrap;gap:.5rem;align-items:center;justify-content:center;padding:.75rem 1rem; }
        .cookie-section { border:1px solid var(--border);border-radius:.5rem;overflow:hidden;margin-bottom:1rem; }
        .cookie-section-header { background:var(--bg2);padding:.75rem 1rem;font-weight:700;font-size:.875rem;color:var(--text); }
        .btn-allow { background:#16a34a;color:white;border:none;padding:.35rem .9rem;border-radius:9999px;font-size:.8rem;font-weight:700;cursor:pointer;white-space:nowrap; }
        .btn-allow:hover { opacity:.85; }
        .btn-deny  { background:#dc2626;color:white;border:none;padding:.35rem .9rem;border-radius:9999px;font-size:.8rem;font-weight:700;cursor:pointer;white-space:nowrap; }
...
<header class="navbar shadow-sm">
    ...
    <!-- Barre supérieure : date sur 1ère ligne, liens sur 2ème avec scroll horizontal -->
    <div style="border-bottom:1px solid var(--border)">
 
        <!-- Date -->
        <div class="max-w-7xl mx-auto px-4 pt-1.5 text-xs" style="color:var(--muted);font-family:'Source Sans 3',sans-serif;">
            <?php
        ...
        <!-- Actions — scroll horizontal sur mobile, rien ne déborde -->
        <div class="max-w-7xl mx-auto px-4 pb-1.5 overflow-x-auto" style="scrollbar-width:none;">
            <div class="flex items-center gap-2 text-xs py-1" style="font-family:'Source Sans 3',sans-serif;color:var(--muted);min-width:max-content;">
                ...
                <!-- Toggle -->
                <button class="theme-toggle ml-1" onclick="toggleTheme()" title="Changer le thème" aria-label="Changer le thème">
        ...
        <a href="<?= BASE_URL ?>/" class="group flex items-center justify-center gap-4">
            ...
            <div class="text-left min-w-0">
                ...
                <div class="flex items-center gap-3 mt-1">
                    <div class="h-px w-8 shrink-0" style="background:linear-gradient(to right,#1d8fd8,#22d3ee)"></div>
                    <span class="text-xs tracking-widest uppercase truncate" style="color:var(--muted);font-family:'Source Sans 3',sans-serif;">Actualités · Culture · Vie locale</span>
                </div>
            </div>
        </a>
    </div>
    ...
    <nav class="max-w-7xl mx-auto px-4" style="border-top:1px solid var(--border)">
        <ul class="flex items-center gap-6 py-3 overflow-x-auto" style="font-family:'Source Sans 3',sans-serif;scrollbar-width:none;">
            <?php
            $navItems = [
                ['url'=>'/','label'=>'Accueil'],
                ['url'=>'/blog','label'=>'Blog'],
    ...
    <div class="max-w-7xl mx-auto px-4 py-10">
        <div class="grid grid-cols-1 md:grid-cols-4 gap-8">
            ...
            <div class="flex flex-col items-start gap-3">
                <img src="<?= BASE_URL ?>/assets/images/Logo_Blog_couleur_avec_fond_blanc.png"
                     alt="Logo" class="rounded-full shadow" style="width:64px;height:64px;object-fit:cover;">
                <div>
            ...
            <div>
                ...
                <ul class="space-y-2 text-sm" style="color:var(--text2);font-family:'Source Sans 3',sans-serif;">
                    ...
                    <li><button onclick="document.getElementById('cookie-banner').style.display='block'" class="hover:text-brand1 transition-colors text-left">Gérer mes cookies</button></li>
                </ul>
            </div>
            ...
            <div>
                ...
                <div class="flex flex-wrap gap-3">
                    ...
                    <a href="https://www.<?= strtolower($rs) ?>.com" target="_blank" rel="noopener noreferrer"
                       ...
                       style="width:56px;height:56px;border-radius:14px;overflow:hidden;display:block;">
                        <img src="<?= BASE_URL ?>/assets/icones/<?= $rs ?>.png" alt="<?= $rs ?>" style="width:100%;height:100%;object-fit:cover;">
                    </a>
                    <?php endforeach; ?>
                </div>
                <p class="text-xs mt-4" style="color:var(--muted);font-family:'Source Sans 3',sans-serif;">Rejoignez notre communauté locale !</p>
            </div>
        </div>
...
function applyTheme(dark) {
    ...
    if (sun && moon) { sun.style.display = dark?'none':'inline'; moon.style.display = dark?'inline':'none'; }
}
...
(function(){ const s=localStorage.getItem('theme'); applyTheme(s?s==='dark':window.matchMedia('(prefers-color-scheme:dark)').matches); })();
 
const COOKIE_KEY = 'bwa_cookies_choice';
function cookieAccept()  { localStorage.setItem(COOKIE_KEY,JSON.stringify({choice:'accepted',youtube:true,vimeo:true,dailymotion:true}));hideCookieBanner(); }
function cookieDeny()    { localStorage.setItem(COOKIE_KEY,JSON.stringify({choice:'denied',youtube:false,vimeo:false,dailymotion:false}));hideCookieBanner(); }
function cookieDetails() { hideCookieBanner();document.getElementById('cookie-modal').style.display='flex'; }
...
function saveCookieChoices(all=null) {
    ...
    ['youtube','vimeo','dailymotion'].forEach(s=>{ c[s]=all===null?(document.querySelector('.cookie-btn-allow-'+s)?.style.outline!==''):all; });
    localStorage.setItem(COOKIE_KEY,JSON.stringify(c));
    closeCookieModal();
}
function hideCookieBanner() { document.getElementById('cookie-banner').style.display='none'; }
(function(){ if(!localStorage.getItem(COOKIE_KEY)) setTimeout(()=>{document.getElementById('cookie-banner').style.display='block';},1200); })();
</script>
